    ...
    async function sendTask() {
        ...
        try {
            const response = await fetch(url, {
                ...
                body: JSON.stringify({ habit: task})
            })
            if (!response.ok) {
            ...
            setSlmResponse(json);
            setSubTasks(json.length > 5 ? json.slice(0,5) : json);
            console.log('slm response',  json);
        } catch(error) {
        ...
        <View style={styles.container}>
            ...
            <View style={styles.addBox}>
                    ...
                    <>
                        {
                            subTasks.map(
                                (subTask, index) =>
                                <TextInput style={styles.subInput} key={"subtask" + index} placeholder={"Add a subtask"} onChangeText={() => handleSubTaskChange(index, event)} value={subTask}></TextInput>
                            )
                        }
                ...
                <View style={styles.btnRow}>
                    ...
                    <TouchableOpacity style={styles.subBtn} onPress={() => sendTask()}>
                        <Text style={styles.subBtnText}>Save</Text>
                    </TouchableOpacity>
                </View>
